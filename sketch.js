...
function preload() {
  ...
  if (phrase != "Empty") {
    ...
    } else imgPhrase = loadImage(phrase.substring(10, phrase.length) + ".png");
  }
...
function getPacificHour() {
  ...
  let pacificHourString = new Intl.DateTimeFormat("en-US", options).format(
    currentDate
  );
...
function draw() {
  ...
  if (level == 1) image(imgcatepillar, 50, 250, 400, 150);
  if (level == 2) image(imgC, 50, 100, 400, 450);
  if (level == 3) image(imgB, 50, 100, 400, 450);
  fill("white");
  textSize(30);
  textFont("Raleway");
...
function gotSpeech() {
  ...
  if (myRec.resultValue) {
    ...
    // var result;
    if (level == 1) {
      if (SorryCount != 1) result = myRec.resultString === food;
    }
    if (level == 2) {
      // console.log("hello");
      if (SorryCount != 1) result = myRec.resultString === phrase;
    ...
    if (result == false) SorryCount++;

    if (result && !goodJobPlaying) {
      // Check the flag to prevent multiple plays
      showGoodJob = true;
      ...
      GoodJob.onended(function () {
        ...
        if (
          counts.breakfastCount < 5 &&
          counts.lunchCount < 5 &&
          counts.dinnerCount < 5 &&
          counts.snackCount < 5
        ) {
